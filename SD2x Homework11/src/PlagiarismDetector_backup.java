...
public class PlagiarismDetector_backup {
	...
	public static Map<String, Integer> detectPlagiarism(String dirName, int windowSize, int threshold) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			for (int j = 0; j < files.length; j++) { 
				...
				Set<String> file1Phrases= null;
				Set<String> file2Phrases= null;
				try {
				MyThread thread1 = new MyThread(dirName, windowSize);
				MyThread thread2 = new MyThread(dirName, windowSize);
				thread1.setFile(file1);
				thread2.setFile(file2);
				thread1.start();
				thread2.start();
				thread1.join();
				thread2.join();
				file1Phrases= thread1.getResult();
				file2Phrases= thread2.getResult();
				}
				catch (InterruptedException e) {
					System.out.println("Threads collided.");
				}
	...
	public static class MyThread extends Thread {
		...
		private Set<String> filePhrases= new HashSet<String>();
		private String file=null;
		
		public MyThread(String dirName, int windowSize) {
		...
		public void run () {
			if (file==null)
				return;
			filePhrases = createPhrases(dirName + "\\" + file, windowSize); 
		}
	 ...
	 */
	protected static synchronized List<String> readFile(String filename) {
    ...
    public static void main(String[] args) {
    	...
    	Map<String, Integer> map = PlagiarismDetector_backup.detectPlagiarism(directory, 4, 1);
